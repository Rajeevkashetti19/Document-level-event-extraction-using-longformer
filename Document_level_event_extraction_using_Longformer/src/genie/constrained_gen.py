...
from transformers.models.led.modeling_led  import LEDPreTrainedModel,LEDSeq2SeqLMOutput

class LEDForConditionalGen(LEDPreTrainedModel):
    def __init__(self, config, tokenizer):
        super(LEDForConditionalGen, self).__init__(config)
        self.config = config 
        self.tokenizer = tokenizer 
        #self.transformer = BartModel.from_pretrained('facebook/bart-large')
        self.transformer= LEDModel.from_pretrained("patrickvonplaten/led-large-16384-pubmed")
    ...
    def forward(self, input_ids, 
        ...
        if task==-1:
            ...
            if inputs_embeds==None:
                ...
                inputs_embeds = self.transformer.encoder.embed_tokens(input_ids) * 1.0
                #self.transformer.encoder.embed_scale #(batch, seq_len, input_seq_len)
            pointer_logits = torch.einsum('ijk,ilk->ijl', decoder_output, inputs_embeds) #(batch, seq_len, input_seq_len)
    ...
    def generate(
        ...
        **model_kwargs
        ...
        if self.config.is_encoder_decoder:
            ...
            inputs_embeds = encoder.embed_tokens(input_ids)  * 1.0
            #encoder.embed_scale 
