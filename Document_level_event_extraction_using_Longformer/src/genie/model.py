...
from transformers import LEDModel, LEDConfig,LEDTokenizer
from .network import BartGen
from .constrained_gen import LEDForConditionalGen
...
class GenIEModel(pl.LightningModule):
    def __init__(self, args):
        ...
        self.config=LEDConfig.from_pretrained('patrickvonplaten/led-large-16384-pubmed')
        self.tokenizer = LEDTokenizer.from_pretrained("patrickvonplaten/led-large-16384-pubmed")
        ...
        if self.hparams.model=='gen':
            ...
            self.model = LEDForConditionalGen(self.config, self.tokenizer)
            self.model.resize_token_embeddings() 
        else:
